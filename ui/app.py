...
# Ensure nest_asyncio is applied once if needed for Streamlit's environment
# This helps run async ADK code within Streamlit's sync context
if 'nest_asyncio_applied' not in st.session_state:
     try:
         ...
         logger.warning("nest_asyncio not found. Async operations might behave unexpectedly.")
         st.session_state.nest_asyncio_applied = False # Mark as attempted but failed
     except RuntimeError as e:
         if "cannot apply nest_asyncio" in str(e):
            logger.info("nest_asyncio already applied or cannot be applied again.")
            st.session_state.nest_asyncio_applied = True # Assume it's effectively applied
         else:
            logger.error(f"Error applying nest_asyncio: {e}")
            st.session_state.nest_asyncio_applied = False


def get_or_create_adk_session(user_id: str) -> str:
    """Gets existing or creates a new ADK session ID for a Streamlit user."""
    # Use Streamlit session state to track the active ADK session ID for this user
    ...
    if session_key in st.session_state:
        ...
        logger.info(f"--- Streamlit ADK: Reusing existing ADK session for {user_id}: {session_id} ---")
        # Verify/recreate in InMemorySessionService if needed (instances might reset)
        try:
            # Run sync ADK session methods using asyncio event loop if available
            # Or run directly if nest_asyncio allows it
            existing = asyncio.run(service.get_session_async(APP_NAME, user_id, session_id))

            if not existing:
                logger.warning(f"--- Streamlit ADK: Session {session_id} for {user_id} not found in service. Recreating.")
                asyncio.run(service.create_session_async(APP_NAME, user_id, session_id, {}))
        except Exception as e:
             logger.error(f"--- Streamlit ADK: Error checking/recreating session {session_id} for {user_id}: {e}")
             # Fallback: Force creation of a new session ID if check fails badly
             if session_key in st.session_state:
                 del st.session_state[session_key]
             return get_or_create_adk_session(user_id) # Recurse to create new
        return session_id
    else:
        # Create new session
        session_id = f"{ADK_SESSION_PREFIX}{int(time.time())}_{os.urandom(4).hex()}"
        st.session_state[session_key] = session_id
        logger.info(f"--- Streamlit ADK: Generated new ADK session for {user_id}: {session_id} ---")
        try:
             # Run sync ADK session methods using asyncio event loop
             asyncio.run(service.create_session_async(APP_NAME, user_id, session_id, {}))
             logger.info(f"--- Streamlit ADK: Successfully created new session {session_id} in SessionService.")
        except Exception as e:
            logger.exception(f"--- Streamlit ADK: FATAL ERROR creating session {session_id} for {user_id}:")
            # Handle error appropriately, maybe remove from active_sessions and raise
            if session_key in st.session_state:
                 del st.session_state[session_key]
...
async def run_adk_turn_async(user_id: str, session_id: str, user_message_text: str) -> str:
    """Runs a single turn with the ADK agent asynchronously."""
    logger.info(f"--- Streamlit ADK Run Async: Session {session_id}, User: {user_id}, Query: '{user_message_text[:100]}...' ---")
        ...
        async for event in adk_runner.run_async(user_id=user_id, session_id=session_id, new_message=content):
            if event.is_final_response():
                if event.content and event.content.parts and hasattr(event.content.parts[0], 'text'):
                    ...
                    logger.info(f"--- Streamlit ADK Run Async: Session {session_id} - Final response received.")
                else:
                    final_response_text = "[Agent finished with no text output]"
                    logger.warning(f"--- Streamlit ADK Run Async: Session {session_id} - Final event has no text content.")
                break # Exit loop once final response is found
    except Exception as e:
        logger.exception(f"--- Streamlit ADK Run Async: Session {session_id} - !! EXCEPTION during agent execution:")
        final_response_text = f"Sorry, an error occurred while processing your request: {e}"

    logger.info(f"--- Streamlit ADK Run Async: Session {session_id} - Turn complete. Response: '{final_response_text[:100]}...' ---")
...
try:
     adk_session_id = get_or_create_adk_session(st.session_state.user_id)
except Exception as e:
...
if prompt := st.chat_input("What would you like to do? (e.g., 'list vms', 'run query select * from `project.dataset.table` limit 5')"):
    ...
    with st.chat_message("assistant"):
        ...
        # Run the ADK turn asynchronously
        try:
            # Use asyncio.run() to execute the async function
            agent_response = asyncio.run(
                run_adk_turn_async(st.session_state.user_id, adk_session_id, prompt)
            )
...
if st.sidebar.button("Clear Chat History"):
    ...
    st.session_state.messages = []
    # Optionally: Consider if you want to explicitly delete/reset the ADK session
    # on the server side here as well, although InMemorySessionService might handle it.
    st.rerun()

if st.sidebar.button("Clear Session State (Debug)"):
    logger.warning("Clearing full Streamlit session state.")
    # Save keys to keep if necessary (like user_id maybe?)
