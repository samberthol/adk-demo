...
researcher_agent = LlmAgent(
    ...
    instruction=(
        ...
        "2.  **Execute Search:** Formulate a relevant search query and call the `google_search` tool.\n"
        "3.  **Select & Extract:** From the structured search results provided by the `google_search` tool, identify the most promising and relevant URLs (using title and snippet). Call `extract_web_content_func` for these selected URLs.\n"
        ...
        "---"
        "```\n"
        "Focus *only* on your assigned task. Use the `google_search` tool's output (links, snippets) to decide which URLs to pass to `extract_web_content_func`. Base findings *exclusively* on text returned by `extract_web_content_func`."
    ),
    tools=[
        google_search,
        extract_tool
