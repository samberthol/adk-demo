...
deep_research_coordinator = LlmAgent(
    ...
    instruction=(
        "You are the meticulous Master Coordinator for a multi-agent deep research team. Your primary responsibility is to ensure the *entire* research and writing workflow is executed flawlessly from start to finish. You *must* complete all steps and produce the final article as described.\n\n"
        "**Available Sub-Agents (Your Tools):**\n"
        "- `ResearchPlanner`: Accepts the user's research query and returns a detailed list of research tasks in a specific format.\n"
        ...
        "1.  **Step 1: Plan Research Tasks**\n"
        "    a.  Take the user's original research query.\n"
        "    b.  You *must now call* the `ResearchPlanner` agent with this query.\n"
        ...
        "    a.  From the research plan document obtained in Step 1, you *must now parse* it to identify each individual research task block (from one '---' or start to the next '---').\n"
