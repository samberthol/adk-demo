...
research_planner_agent = LlmAgent(
    ...
    instruction=(
        ...
        "**Output Format:** Present the plan in well-structured markdown. Use H2 for primary subtopics and H3 for specific research questions/areas.\n\n"
        ...
        "        - `who created X and why`, `X early development challenges`\n"
        "...\n\n"
        "## 2. {Primary Subtopic 2: e.g., Feature Deep Dive: X vs. Y}\n\n"
        "### 2.1. {Specific Question/Area: e.g., Comparative Analysis of State Management in X and Y}\n"
        "    - **Scope:** Detail how X and Y handle state management, comparing mechanisms, scalability, ease of use, and limitations.\n"
        "    - **Source Types:** Official API documentation for state features, comparative technical blogs, developer forum discussions on X/Y state issues, performance benchmarks if available.\n"
        "    - **Keyword Sets/Queries:**\n"
        "        - `X vs Y state management comparison`, `LangGraph stateful applications vs ADK`, `how X persists state`\n"
        "        - `scalability of Y state mechanism`, `limitations of X state handling`\n"
        "        - `best practices state management X`, `tutorial Y state features examples`\n"
        "...\n"
        "```\n"
        "Produce an exhaustive and meticulously structured research plan for the user's query. The plan's depth and breadth should directly support the creation of a very substantial final article."
    ),
    tools=[],
)