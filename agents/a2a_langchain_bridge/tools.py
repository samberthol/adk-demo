...
class LangchainA2ATool(Tool):
    """
    Tool to communicate with an external LangGraph agent via the A2A JSON-RPC protocol.
    Implements createTask and polls getTask. Authentication is omitted for simplicity.
    """
    name: str = "LangchainA2AJsonRpcClient"
    description: str = (
    ...
    def _send_json_rpc_request(self, method: str, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def __call__(self, query: str) -> str:
        """
        Creates a task via A2A JSON-RPC, polls for completion, and returns the result.

        Args:
            query: The natural language query for the LangGraph agent.

        Returns:
            The response text from the LangGraph agent, or an error message.
        """
        try:
            # --- 1. Create Task ---
            ...
            create_response = self._send_json_rpc_request("createTask", create_params)
            ...
            if not task_id:
                ...
                return "Error: Failed to create task on LangGraph agent (missing taskId)."
            ...
            for attempt in range(MAX_POLLING_ATTEMPTS):
                ...
                get_response = self._send_json_rpc_request("getTask", get_params)
                ...
                if task_state == "completed":
                    ...
                    return "Error: LangGraph agent completed the task but provided no text response."
                     ...
                     return f"Error: LangGraph agent task {task_state}: {error_message}"

            logger.warning(f"Task {task_id} did not complete within the polling timeout.")
            return "Error: The request to the LangGraph agent timed out waiting for task completion."
            ...
            logger.exception(f"An unexpected error occurred in LangchainA2ATool __call__:")
            return f"Error: An unexpected error occurred during A2A communication."

# Instantiate the tool for use by the agent
langchain_a2a_tool = LangchainA2ATool()