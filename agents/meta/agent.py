...
meta_agent = LlmAgent(
    ...
    instruction=(
        ...
        "**Routing Logic:**\n"
        "- If the request involves **in-depth research**, **investigation**, creating a **detailed report**, or analyzing a complex topic: Delegate to `DeepResearchCoordinatorAgent`. **Invoke the transfer using *only* the agent name (`DeepResearchCoordinatorAgent`). Do NOT pass the user query as an argument to the transfer function.** The target agent will access the query from the conversation history.\n"
        "- If it involves managing cloud resources (VMs), delegate to `ResourceAgent` (provide only the agent name).\n"
        "- If it involves BigQuery data or datasets, delegate to `DataScienceAgent` (provide only the agent name).\n"
        "- If it involves GitHub repositories or files, delegate to `githubagent` (provide only the agent name).\n"
        f"- If it asks about GCP services, documentation, or needs GCP fact-checking, delegate to `{LLM_AUDITOR_NAME}` (provide only the agent name).\n"
        f"- If the request involves currency conversion or exchange rates, use the tool `langgraph_currency_a2a_tool_func`. Provide the user's query text to the tool's `query` parameter.\n"
        f"- For general conversation, summarization, translation, or if no other specialist agent fits, delegate to `{MISTRAL_AGENT_NAME}` (provide only the agent name).\n\n"
        "**Execution:**\n"
        "- When delegating to another agent, invoke the transfer function call specifying *only* the target agent's name.\n"
        "- When using a tool, call the specific tool function with its required parameters (e.g., `query` for the currency tool).\n"
