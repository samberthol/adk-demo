...
# --- Modified Async Runner Function --- ## MODIFIED HERE ##
async def run_adk_async(runner: Runner, session_id: str, user_id: str, user_message_text: str) -> Tuple[str, str, Set[str]]:
    ...
    # Initialize author trackers
    final_response_author = "assistant" # Overall final author default
    last_text_event_author = "assistant" # Author of the most recent text event
    ...
    try:
        async for event in runner.run_async(user_id=user_id, session_id=session_id, new_message=content):
            ...
            # Check specifically for text content to update final text and its author
            if event.content and event.content.parts and hasattr(event.content.parts[0], 'text'):
                 final_response_text = event.content.parts[0].text
                 last_text_event_author = author or "assistant" # Track author of this text event
        ...
        if "Session not found" in str(e):
             ...
             last_text_event_author = "error" # Set author on error
        ...
        # Use the author from the LAST text event as the final author for highlighting
        final_response_author = last_text_event_author
        logger.info(f"--- ADK Run Async: Turn execution completed in {duration:.2f} seconds.")
        logger.info(f"--- ADK Run Async: Final Author (for UI): {final_response_author}")
        logger.info(f"--- ADK Run Async: Activated agents this turn: {activated_agents_set}")


    # Return final text, the author of the LAST text event, and the set
...
def generate_mermaid_syntax(root_agent_name: str, activated_agents: Set[str], last_author: str = None) -> str:
    ...
    try:
        ...
        if agents_to_draw and agents_to_draw != {"error"}:
             if root_agent_name not in agents_to_draw:
